...
async def user_message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.message.photo:
        ...
        # Check if it's a GIF by file extension or MIME type
        is_gif = file.file_path.lower().endswith('.gif') or 'gif' in file.file_path.lower()
        if is_gif:
...
def chat_send(user_id):
    ...
    if files and len(files) > 0:
        ...
        for file in files:
            ...
            # Check if it's a GIF
            is_gif = filename.lower().endswith('.gif') or mimetype == 'image/gif'
            
            if mimetype.startswith('image/'):
                if is_gif:
                    # Handle GIF as video for better compatibility
        ...
        if sent:
            ...
            response = {'status': 'error', 'message': 'Failed to send media'}
